...
class _SleepScreenState extends State<SleepScreen> {
  ...
  Duration? get _weeklyAverage {
    ...
    final totalMinutes = _recentLogs.fold<int>(
      0,
  ...
  // past-midnight bedtime (extremely common) got a bedtime date a full day
  // too early, inflating the logged duration by ~24h.
  (DateTime bedtimeDate, DateTime wakeTimeDate) _resolveDates(
    DateTime wakeDay,
    TimeOfDay bedtime,
    TimeOfDay wakeTime,
  ) {
    ...
    final bedtimeDay = bedtime.hour < 12
        ? wakeDay
    ...
    return (bedtimeDate, wakeTimeDate);
  }
  ...
  Future<void> _logManually() async {
    ...
    final (bedtimeDate, wakeTimeDate) = _resolveDates(today, bedtime, wakeTime);

    await _save(bedtimeDate, wakeTimeDate);
  }

  Future<void> _editPastNight(SleepLog log) async {
    final sleepDate = log.sleepDate;
    if (sleepDate == null) return;

    final picked = await showSleepDialSheet(
      context,
      initialBedtime: TimeOfDay.fromDateTime(log.bedtime),
      initialWakeTime: TimeOfDay.fromDateTime(log.wakeTime),
    );
    if (picked == null || !mounted) return;
    final (bedtime, wakeTime) = picked;

    final wakeDay = DateTime.parse(sleepDate);
    final (bedtimeDate, wakeTimeDate) = _resolveDates(
      wakeDay,
      bedtime,
      wakeTime,
    );
        ...
        child: _loading
            ...
            : ListView(
                ...
                children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          _lastNight == null
                              ? '—'
                              : _formatDuration(_lastNight!.duration),
                  ...
                  Text(
                    strings.ultime7Notti,
                    style: theme.textTheme.labelMedium,
                  ),
                  const SizedBox(height: 4),
                  if (_recentLogs.isNotEmpty)
                    Text(
                      strings.tocca7NottiPerCorreggere,
...
class _SleepTimeline extends StatelessWidget {
  const _SleepTimeline({required this.logs, required this.onEditLog});

  final List<SleepLog> logs;
  final ValueChanged<SleepLog> onEditLog;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: logs.map((log) {
        ...
        final startFraction =
            (log.bedtime.difference(reference).inMinutes / (_spanHours * 60))
                .clamp(0.0, 1.0);
        final endFraction =
            (log.wakeTime.difference(reference).inMinutes / (_spanHours * 60))
                .clamp(0.0, 1.0);
        ...
        return InkWell(
          ...
          onTap: log.sleepDate == null ? null : () => onEditLog(log),
          child: Padding(
                      ...
                      return Stack(
                        children: [
                          ...
                          Positioned(
                            ...
                            width: (w * (endFraction - startFraction)).clamp(
                              4.0,
                              w,
                            ),
                            child: Container(
